...
import initParquetWasm, { Table as ParquetTable, writeParquet } from 'parquet-wasm';
import './ResultsTable.css';

// Initialize parquet-wasm (once)
let parquetInitialized = false;
const initParquet = async () => {
...
export function ResultsTable({ result, maxHeight, onPageChange, isLoading }: ResultsTableProps) {
  ...
  return (
    <div className="results-table-container">
      <div className="results-table-wrapper" style={tableStyle}>
        {isLoading && (
        ...
        <table className="results-table">
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map((row, rowIdx) => (
                <tr key={rowIdx}>
                  <td className="row-number">{rowOffset + rowIdx + 1}</td>
